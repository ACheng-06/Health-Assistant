<template>
  <div>
    ...
    <el-table :data="tableData" style="width: 100%; margin-top: 25px;">
      <el-table-column prop="nickname" label="用户名" width="80" :align="`left`" />
      ...
      <el-table-column label="情绪评分" width="200">
        <template #default="scope">
          <el-rate :model-value="scope.row.moodScore" :key="scope.row.moodScore" :max="10" disabled />
        </template>
      </el-table-column>
      ...
      <el-table-column label="生活指标" width="120">
        <template #default="scope">
          <div>
            <p style="margin: 0; line-height: 1.5;">睡眠：{{ scope.row.sleepQuality || '未填写' }}</p>
            <p style="margin: 0; line-height: 1.5;">压力：{{ scope.row.stressLevel || '未填写' }}</p>
          </div>
        </template>
      </el-table-column>
    ...
    <el-pagination style="margin-top: 25px;" :current-page="pagination.current" :page-size="pagination.size"
      layout="prev, pager, next" :total="pagination.total" @current-change="handleChange" />

    <el-dialog v-model="detailDialogVisble" title="情绪日志详情" width="880px" :close-on-click-modal="false">
      <div class="detail-content" v-if="currentDetail">
        ...
        <div class="detail-section">
          ...
          <el-descriptions :column="2" border>
            <el-descriptions-item label="情绪评分">
              <el-rate :model-value="currentDetail.moodScore" :key="currentDetail.moodScore" :max="10" disabled />
        ...
        <div class="detail-section">
          <h4>AI情绪分析结果</h4>
          <div v-if="aiData && aiData.primaryEmotion" class="ai-analysis-result">
            <el-descriptions :column="2" border>
              <el-descriptions-item label="主要情绪">
          ...
          <div v-else style="color: #909399; padding: 16px; text-align: center;">暂无AI分析结果</div>
        </div>
        <div class="detail-section">
          <h4>时间信息</h4>
          <el-descriptions :column="2" border>
...
<script setup>
  ...
  import { getEmotionalPageAPI, deleteEmotionalAPI } from '@/api/admin'
  ...
  const formItem = ref([
    ...
    {
      comp: 'select', prop: 'moodScreRange', label: '情绪评价', placeholder: '请选择评分范围', options: [
        { label: '低(1-3分)', value: '0-3' },
        { label: '中(4-6分)', value: '4-6' },
        { label: '高(7-10分)', value: '7-10' }
      ]
    }
  ])
  ...
  const handleSearch = async (formData) => {
    ...
    if (params.moodScreRange) {
      const [min, max] = params.moodScreRange.split('-')
      params.minMoodScore = min
      params.maxMoodScore = max
      delete params.moodScreRange
    }

    try {
      const res = await getEmotionalPageAPI(params)
